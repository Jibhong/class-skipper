...
export async function GET(req: Request) {
  ...
  const username = ok;
  console.log("GET api/get-firebase-token, token valid:", ok);
  if (!ok || typeof username !== "string") {
    return NextResponse.json({ error: "Invalid credentials" }, { status: 401 });
  }
  const tokenRes = await getFirebaseToken(username);
  if (!tokenRes) {
